...
{    
    public class DiseaseMaster : IModel,INotifyPropertyChanged
    {
        private long     nDiseaseCode;
        public long     DiseaseCode
        {
            get { return nDiseaseCode; }
            set
            {
                nDiseaseCode = value;
        ...
        #region IViewDetail Members
            ...
            {
                if (!DBReader.IsDBNull(DBReader.GetOrdinal("DiseaseCode"))){
                    this.DiseaseCode = DBReader.GetInt64(DBReader.GetOrdinal("DiseaseCode"));
                }
                if (!DBReader.IsDBNull(DBReader.GetOrdinal("ICode"))){
                    this.ICode = DBReader.GetString(DBReader.GetOrdinal("ICode"));
                }
                if (!DBReader.IsDBNull(DBReader.GetOrdinal("DiseaseName"))){
                    this.DiseaseName = DBReader.GetString(DBReader.GetOrdinal("DiseaseName"));
                }
        ...
        {  
            ...
            sXml.Append("<data>");
            sXml.Append("<DiseaseCode>" + this.DiseaseCode + "</DiseaseCode>");
            sXml.Append("<ICode>" + this.ICode + "</ICode>");
            sXml.Append("<DiseaseName>" + this.DiseaseName + "</DiseaseName>");
        ...
        {  
            ...
            this.DiseaseCode = objdata.DiseaseCode;
            this.ICode = objdata.ICode;
            this.DiseaseName = objdata.DiseaseName;
        ...
        {  
            T obj = new T();
            DiseaseMaster objdata = obj as DiseaseMaster;
            objdata.DiseaseCode = this.DiseaseCode;
            objdata.ICode = this.ICode;
            objdata.DiseaseName = this.DiseaseName;
