...
{    
     ...
     {    
            public void GetListXMaster<T>(T objFilter, ref List<T> listData) where T : class, IModel, new()
            {
                string sQuery = "GetListXMaster";
                XMaster objData = objFilter as XMaster;
                ...
                SqlConnManager.GetList<T>(sQuery,CommandType.StoredProcedure,list.ToArray(), FillXMasterDataFromReader, ref  listData);
            }
            
            private void FillXMasterDataFromReader<T>(DbDataReader DbReader, ref List<T> listData) where T : class, IModel, new()
            {
                while (DbReader.Read())
                {
                    T obj = new T();
                    XMaster objData = obj as XMaster;
                    obj.FillDataFromDB(DbReader);
                    listData.Add(obj);
                }
            }
  
            public DataBaseResultSet SaveXMaster<T>(T objData) where T : class, IModel, new()
            {
                XMaster obj = objData as XMaster;
                string sQuery = "sprocXMasterInsertUpdateSingleItem";
                List<DbParameter> list = new List<DbParameter>();
