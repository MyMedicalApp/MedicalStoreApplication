...
{    
     ...
     {    
            public void GetListoVatDetail<T>(T objFilter, ref List<T> listData) where T : class, IModel, new()
            {
                string sQuery = "GetListoVatDetail";
                oVatDetail objData = objFilter as oVatDetail;
                ...
                SqlConnManager.GetList<T>(sQuery,CommandType.StoredProcedure,list.ToArray(), FilloVatDetailDataFromReader, ref  listData);
            }
            
            private void FilloVatDetailDataFromReader<T>(DbDataReader DbReader, ref List<T> listData) where T : class, IModel, new()
            {
                while (DbReader.Read())
                {
                    T obj = new T();
                    oVatDetail objData = obj as oVatDetail;
                    obj.FillDataFromDB(DbReader);
                    listData.Add(obj);
                }
            }
  
            public DataBaseResultSet SaveoVatDetail<T>(T objData) where T : class, IModel, new()
            {
                oVatDetail obj = objData as oVatDetail;
                string sQuery = "sprocoVatDetailInsertUpdateSingleItem";
                List<DbParameter> list = new List<DbParameter>();
