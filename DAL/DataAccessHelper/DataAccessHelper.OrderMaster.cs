...
{    
     ...
     {    
            public void GetListOrderMaster<T>(T objFilter, ref List<T> listData) where T : class, IModel, new()
            {
                string sQuery = "GetListOrderMaster";
                OrderMaster objData = objFilter as OrderMaster;
                ...
                SqlConnManager.GetList<T>(sQuery,CommandType.StoredProcedure,list.ToArray(), FillOrderMasterDataFromReader, ref  listData);
            }
            
            private void FillOrderMasterDataFromReader<T>(DbDataReader DbReader, ref List<T> listData) where T : class, IModel, new()
            {
                while (DbReader.Read())
                {
                    T obj = new T();
                    OrderMaster objData = obj as OrderMaster;
