...
{    
     ...
     {    
            public void GetListOldBillMaster<T>(T objFilter, ref List<T> listData) where T : class, IModel, new()
            {
                string sQuery = "GetListOldBillMaster";
                OldBillMaster objData = objFilter as OldBillMaster;
                ...
                SqlConnManager.GetList<T>(sQuery,CommandType.StoredProcedure,list.ToArray(), FillOldBillMasterDataFromReader, ref  listData);
            }
            
            private void FillOldBillMasterDataFromReader<T>(DbDataReader DbReader, ref List<T> listData) where T : class, IModel, new()
            {
                while (DbReader.Read())
                {
                    T obj = new T();
                    OldBillMaster objData = obj as OldBillMaster;
                    obj.FillDataFromDB(DbReader);
                    listData.Add(obj);
                }
            }
  
            public DataBaseResultSet SaveOldBillMaster<T>(T objData) where T : class, IModel, new()
            {
                OldBillMaster obj = objData as OldBillMaster;
                string sQuery = "sprocOldBillMasterInsertUpdateSingleItem";
                List<DbParameter> list = new List<DbParameter>();
