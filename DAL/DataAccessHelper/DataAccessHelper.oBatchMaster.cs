...
{    
     ...
     {    
            public void GetListoBatchMaster<T>(T objFilter, ref List<T> listData) where T : class, IModel, new()
            {
                string sQuery = "GetListoBatchMaster";
                oBatchMaster objData = objFilter as oBatchMaster;
                ...
                SqlConnManager.GetList<T>(sQuery,CommandType.StoredProcedure,list.ToArray(), FilloBatchMasterDataFromReader, ref  listData);
            }
            
            private void FilloBatchMasterDataFromReader<T>(DbDataReader DbReader, ref List<T> listData) where T : class, IModel, new()
            {
                while (DbReader.Read())
                {
                    T obj = new T();
            ...
            public DataBaseResultSet SaveoBatchMaster<T>(T objData) where T : class, IModel, new()
            {
                oBatchMaster obj = objData as oBatchMaster;
                string sQuery = "sprocoBatchMasterInsertUpdateSingleItem";
                List<DbParameter> list = new List<DbParameter>();
