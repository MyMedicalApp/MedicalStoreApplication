...
{    
     ...
     {    
            public void GetListOrderDetail<T>(T objFilter, ref List<T> listData) where T : class, IModel, new()
            {
                string sQuery = "GetListOrderDetail";
                OrderDetail objData = objFilter as OrderDetail;
                ...
                SqlConnManager.GetList<T>(sQuery,CommandType.StoredProcedure,list.ToArray(), FillOrderDetailDataFromReader, ref  listData);
            }
            
            private void FillOrderDetailDataFromReader<T>(DbDataReader DbReader, ref List<T> listData) where T : class, IModel, new()
            {
                while (DbReader.Read())
                {
                    T obj = new T();
