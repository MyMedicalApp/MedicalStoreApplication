...
{    
     ...
     {    
            public void GetListDeletedDetail<T>(T objFilter, ref List<T> listData) where T : class, IModel, new()
            {
                string sQuery = "GetListDeletedDetail";
                DeletedDetail objData = objFilter as DeletedDetail;
                ...
                SqlConnManager.GetList<T>(sQuery,CommandType.StoredProcedure,list.ToArray(), FillDeletedDetailDataFromReader, ref  listData);
            }
            
            private void FillDeletedDetailDataFromReader<T>(DbDataReader DbReader, ref List<T> listData) where T : class, IModel, new()
            {
                while (DbReader.Read())
                {
                    T obj = new T();
                    DeletedDetail objData = obj as DeletedDetail;
                    obj.FillDataFromDB(DbReader);
                    listData.Add(obj);
                }
            }
  
            public DataBaseResultSet SaveDeletedDetail<T>(T objData) where T : class, IModel, new()
            {
                DeletedDetail obj = objData as DeletedDetail;
                string sQuery = "sprocDeletedDetailInsertUpdateSingleItem";
                List<DbParameter> list = new List<DbParameter>();
