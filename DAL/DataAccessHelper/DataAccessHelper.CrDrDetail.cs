...
{    
     ...
     {    
            public void GetListCrDrDetail<T>(T objFilter, ref List<T> listData) where T : class, IModel, new()
            {
                string sQuery = "GetListCrDrDetail";
                CrDrDetail objData = objFilter as CrDrDetail;
                ...
                SqlConnManager.GetList<T>(sQuery,CommandType.StoredProcedure,list.ToArray(), FillCrDrDetailDataFromReader, ref  listData);
            }
            
            private void FillCrDrDetailDataFromReader<T>(DbDataReader DbReader, ref List<T> listData) where T : class, IModel, new()
            {
                while (DbReader.Read())
                {
                    T obj = new T();
                    CrDrDetail objData = obj as CrDrDetail;
                    obj.FillDataFromDB(DbReader);
                    listData.Add(obj);
                }
            }
  
            public DataBaseResultSet SaveCrDrDetail<T>(T objData) where T : class, IModel, new()
            {
                CrDrDetail obj = objData as CrDrDetail;
                string sQuery = "sprocCrDrDetailInsertUpdateSingleItem";
                List<DbParameter> list = new List<DbParameter>();
