...
{    
     ...
     {    
            public void GetListPurDetail<T>(T objFilter, ref List<T> listData) where T : class, IModel, new()
            {
                string sQuery = "GetListPurDetail";
                PurDetail objData = objFilter as PurDetail;
                ...
                SqlConnManager.GetList<T>(sQuery,CommandType.StoredProcedure,list.ToArray(), FillPurDetailDataFromReader, ref  listData);
            }
            
            private void FillPurDetailDataFromReader<T>(DbDataReader DbReader, ref List<T> listData) where T : class, IModel, new()
            {
                while (DbReader.Read())
                {
                    T obj = new T();
                    PurDetail objData = obj as PurDetail;
                    obj.FillDataFromDB(DbReader);
                    listData.Add(obj);
                }
            }
  
            public DataBaseResultSet SavePurDetail<T>(T objData) where T : class, IModel, new()
            {
                PurDetail obj = objData as PurDetail;
                string sQuery = "sprocPurDetailInsertUpdateSingleItem";
                List<DbParameter> list = new List<DbParameter>();
                ...
                list.Add(SqlConnManager.GetConnParameters("CUser", "CUser", 8, GenericDataType.Decimal, ParameterDirection.Input, obj.CUser));
