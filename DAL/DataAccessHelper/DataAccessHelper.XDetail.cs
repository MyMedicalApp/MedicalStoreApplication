...
{    
     ...
     {    
            ...
            private void FillXDetailDataFromReader<T>(DbDataReader DbReader, ref List<T> listData) where T : class, IModel, new()
            {
                while (DbReader.Read())
                {
                    T obj = new T();
            ...
            public DataBaseResultSet SaveXDetail<T>(T objData) where T : class, IModel, new()
            {
                XDetail obj = objData as XDetail;
                string sQuery = "sprocXDetailInsertUpdateSingleItem";
                List<DbParameter> list = new List<DbParameter>();
