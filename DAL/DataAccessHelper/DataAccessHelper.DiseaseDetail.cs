...
{    
     ...
     {    
            public void GetListDiseaseDetail<T>(T objFilter, ref List<T> listData) where T : class, IModel, new()
            {
                string sQuery = "GetListDiseaseDetail";
                DiseaseDetail objData = objFilter as DiseaseDetail;
                ...
                SqlConnManager.GetList<T>(sQuery,CommandType.StoredProcedure,list.ToArray(), FillDiseaseDetailDataFromReader, ref  listData);
            }
            
            private void FillDiseaseDetailDataFromReader<T>(DbDataReader DbReader, ref List<T> listData) where T : class, IModel, new()
            {
                while (DbReader.Read())
                {
                    T obj = new T();
                    DiseaseDetail objData = obj as DiseaseDetail;
                    obj.FillDataFromDB(DbReader);
                    listData.Add(obj);
                }
            }
  
            public DataBaseResultSet SaveDiseaseDetail<T>(T objData) where T : class, IModel, new()
            {
                DiseaseDetail obj = objData as DiseaseDetail;
                string sQuery = "sprocDiseaseDetailInsertUpdateSingleItem";
                List<DbParameter> list = new List<DbParameter>();
