-- Store Procedures for GroupMaster Table
 
CREATE PROC [dbo].[sprocGroupMasterInsertUpdateSingleItem] 
...
BEGIN 
    ...
    BEGIN TRY
        ...
        IF (@OperationFlag = 0)
        BEGIN
                set @GroupCode = (select count(*) from GroupMaster)
                if @GroupCode  <> 0
                    set @GroupCode = (select max(GroupCode) from GroupMaster)
                set @GroupCode = @GroupCode + 1
                ...
                SET @Message=convert(varchar(20),@GroupCode)+' Record Inserted SucessFully'
        END
        ...
        BEGIN
                ...
                SET @Message=convert(varchar(20),@GroupCode)+' Record Update SucessFully'
        END
        ELSE IF (@OperationFlag = 2)
        BEGIN
            DELETE FROM [GroupMaster] WHERE  [GroupCode] = @GroupCode
            SET @Message=convert(varchar(20),@GroupCode)+' Record Deleted SucessFully'
        END
        COMMIT TRANSACTION
    END TRY
