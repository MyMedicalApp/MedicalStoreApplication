-- Store Procedures for DiseaseMaster Table
 
CREATE PROC [dbo].[sprocDiseaseMasterInsertUpdateSingleItem] 
...
BEGIN 
    ...
    BEGIN TRY
        ...
        IF (@OperationFlag = 0)
        BEGIN
                set @DiseaseCode = (select count(*) from DiseaseMaster)
                if @DiseaseCode  <> 0
                    set @DiseaseCode = (select max(DiseaseCode) from DiseaseMaster)
                set @DiseaseCode = @DiseaseCode + 1
                ...
                SET @Message=convert(varchar(20),@DiseaseCode)+' Record Inserted SucessFully'
        END
        ...
        BEGIN
                ...
                SET @Message=convert(varchar(20),@DiseaseCode)+' Record Update SucessFully'
        END
        ELSE IF (@OperationFlag = 2)
        BEGIN
            DELETE FROM [DiseaseMaster] WHERE  [DiseaseCode] = @DiseaseCode
            SET @Message=convert(varchar(20),@DiseaseCode)+' Record Deleted SucessFully'
        END
        COMMIT TRANSACTION
    END TRY
