-- Store Procedures for MapProduct Table
 
CREATE PROC [dbo].[sprocMapProductInsertUpdateSingleItem] 
						@RecNo bigint = NULL,
						@AccountCode bigint = NULL,
						@PartyProdCode varchar(20) = NULL,
						@PartyProdName varchar(50) = NULL,
						@PartyProdPacking varchar(10) = NULL,
...
BEGIN 
    ...
    BEGIN TRY
        ...
        IF (@OperationFlag = 0)
        BEGIN
                set @RecNo = (select count(*) from MapProduct)
                if @RecNo  <> 0
                    set @RecNo = (select max(RecNo) from MapProduct)
                set @RecNo = @RecNo + 1
 
                insert into [MapProduct]
                (
                    [AccountCode],
                    [PartyProdCode],
                    [PartyProdName],
        ...
        BEGIN
                ...
                [PartyProdCode] = @PartyProdCode,
                [PartyProdName] = @PartyProdName,
                [PartyProdPacking] = @PartyProdPacking,
        ...
        BEGIN
            DELETE FROM [MapProduct] WHERE  [RecNo] = @RecNo
