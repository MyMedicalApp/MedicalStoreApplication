-- Store Procedures for CrDrDetail Table
 
CREATE PROC [dbo].[sprocCrDrDetailInsertUpdateSingleItem] 
						@RecNo bigint = NULL,
...
BEGIN 
    ...
    BEGIN TRY
        ...
        IF (@OperationFlag = 0)
        BEGIN
                set @RecNo = (select count(*) from CrDrDetail)
                if @RecNo  <> 0
                    set @RecNo = (select max(RecNo) from CrDrDetail)
                set @RecNo = @RecNo + 1
 
                insert into [CrDrDetail]
                (
                    [BillTag],
                    [BillType],
                    [BillCredit],
        ...
        BEGIN
            DELETE FROM [CrDrDetail] WHERE  [RecNo] = @RecNo
