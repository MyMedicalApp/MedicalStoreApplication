-- Store Procedures for GetList dtproperties Table
 
CREATE PROC [dbo].[GetListdtproperties] 
						@property varchar
AS 
BEGIN 
 declare @Error int,@Message varchar(1000)
 
 
    BEGIN TRY
        BEGIN TRANSACTION
            SELECT dtproperties.* FROM dtproperties

            WHERE @property = 0 OR property = @property
        COMMIT TRANSACTION
    END TRY
    ...
    BEGIN CATCH	
        ...
        exec   sprocErrorDescInsertUpdateSingleItem  0								--@CategoryId int = NULL,
                                                    ...
                                                    ,@Error output				    --@ErrorCode int output
    END CATCH
    ENDPROC:
    print @Message
END
