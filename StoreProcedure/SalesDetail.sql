-- Store Procedures for SalesDetail Table
 
CREATE PROC [dbo].[sprocSalesDetailInsertUpdateSingleItem] 
						@RecNo bigint = NULL,
...
BEGIN 
    ...
    BEGIN TRY
        ...
        IF (@OperationFlag = 0)
        BEGIN
                set @RecNo = (select count(*) from SalesDetail)
                if @RecNo  <> 0
                    set @RecNo = (select max(RecNo) from SalesDetail)
                set @RecNo = @RecNo + 1
        ...
        BEGIN
            DELETE FROM [SalesDetail] WHERE  [RecNo] = @RecNo
