-- Store Procedures for oBatchMaster Table
 
CREATE PROC [dbo].[sprocoBatchMasterInsertUpdateSingleItem] 
...
BEGIN 
    ...
    BEGIN TRY
        ...
        IF (@OperationFlag = 0)
        BEGIN
                set @BatchCode = (select count(*) from oBatchMaster)
                if @BatchCode  <> 0
                    set @BatchCode = (select max(BatchCode) from oBatchMaster)
                set @BatchCode = @BatchCode + 1
 
                insert into [oBatchMaster]
                (
                    [ICode],
                    [ProductCode],
                    [AccountCode],
        ...
        BEGIN
            DELETE FROM [oBatchMaster] WHERE  [BatchCode] = @BatchCode
