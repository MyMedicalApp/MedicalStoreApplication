-- Store Procedures for CompanyDetail Table
 
CREATE PROC [dbo].[sprocCompanyDetailInsertUpdateSingleItem] 
						@RecNo bigint = NULL,
...
BEGIN 
    ...
    BEGIN TRY
        ...
        IF (@OperationFlag = 0)
        BEGIN
                set @RecNo = (select count(*) from CompanyDetail)
                if @RecNo  <> 0
                    set @RecNo = (select max(RecNo) from CompanyDetail)
                set @RecNo = @RecNo + 1
        ...
        BEGIN
            DELETE FROM [CompanyDetail] WHERE  [RecNo] = @RecNo
