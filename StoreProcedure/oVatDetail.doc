-- Store Procedures for oVatDetail Table
 
CREATE PROC [dbo].[sprocoVatDetailInsertUpdateSingleItem] 
						@RecNo bigint = NULL,
...
BEGIN 
    ...
    BEGIN TRY
        ...
        IF (@OperationFlag = 0)
        BEGIN
                set @RecNo = (select count(*) from oVatDetail)
                if @RecNo  <> 0
                    set @RecNo = (select max(RecNo) from oVatDetail)
                set @RecNo = @RecNo + 1
 
                insert into [oVatDetail]
                (
                    [BillTag],
                    [BillType],
                    [BillCredit],
        ...
        BEGIN
            DELETE FROM [oVatDetail] WHERE  [RecNo] = @RecNo
