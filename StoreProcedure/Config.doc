-- Store Procedures for Config Table
 
CREATE PROC [dbo].[sprocConfigInsertUpdateSingleItem] 
						@RecNo bigint = NULL,
						@DfltVariable varchar(50) = NULL,
						@DfltValue varchar(50) = NULL,
...
BEGIN 
    ...
    BEGIN TRY
        ...
        IF (@OperationFlag = 0)
        BEGIN
                set @RecNo = (select count(*) from Config)
                if @RecNo  <> 0
                    set @RecNo = (select max(RecNo) from Config)
                set @RecNo = @RecNo + 1
        ...
        BEGIN
            DELETE FROM [Config] WHERE  [RecNo] = @RecNo
