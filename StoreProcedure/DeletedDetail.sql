-- Store Procedures for DeletedDetail Table
 
CREATE PROC [dbo].[sprocDeletedDetailInsertUpdateSingleItem] 
						@RecNo bigint = NULL,
...
BEGIN 
    ...
    BEGIN TRY
        ...
        IF (@OperationFlag = 0)
        BEGIN
                set @RecNo = (select count(*) from DeletedDetail)
                if @RecNo  <> 0
                    set @RecNo = (select max(RecNo) from DeletedDetail)
                set @RecNo = @RecNo + 1
 
                insert into [DeletedDetail]
                (
                    [RecNo],
                    [BillTag],
                    [BillType],
        ...
        BEGIN
            DELETE FROM [DeletedDetail] WHERE  [RecNo] = @RecNo
