-- Store Procedures for BillMaster Table
 
CREATE PROC [dbo].[sprocBillMasterInsertUpdateSingleItem] 
						@RecNo bigint = NULL,
...
BEGIN 
    ...
    BEGIN TRY
        ...
        IF (@OperationFlag = 0)
        BEGIN
                set @RecNo = (select count(*) from BillMaster)
                if @RecNo  <> 0
                    set @RecNo = (select max(RecNo) from BillMaster)
                set @RecNo = @RecNo + 1
        ...
        BEGIN
            DELETE FROM [BillMaster] WHERE  [RecNo] = @RecNo
