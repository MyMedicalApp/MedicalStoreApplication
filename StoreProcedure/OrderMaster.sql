-- Store Procedures for OrderMaster Table
 
CREATE PROC [dbo].[sprocOrderMasterInsertUpdateSingleItem] 
						@RecNo bigint = NULL,
...
BEGIN 
    ...
    BEGIN TRY
        ...
        IF (@OperationFlag = 0)
        BEGIN
                set @RecNo = (select count(*) from OrderMaster)
                if @RecNo  <> 0
                    set @RecNo = (select max(RecNo) from OrderMaster)
                set @RecNo = @RecNo + 1
        ...
        BEGIN
            DELETE FROM [OrderMaster] WHERE  [RecNo] = @RecNo
