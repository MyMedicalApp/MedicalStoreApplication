-- Store Procedures for XDetail Table
 
CREATE PROC [dbo].[sprocXDetailInsertUpdateSingleItem] 
						@RecNo bigint = NULL,
...
BEGIN 
    ...
    BEGIN TRY
        ...
        IF (@OperationFlag = 0)
        BEGIN
                set @RecNo = (select count(*) from XDetail)
                if @RecNo  <> 0
                    set @RecNo = (select max(RecNo) from XDetail)
                set @RecNo = @RecNo + 1
        ...
        BEGIN
            DELETE FROM [XDetail] WHERE  [RecNo] = @RecNo
