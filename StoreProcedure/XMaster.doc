-- Store Procedures for XMaster Table
 
CREATE PROC [dbo].[sprocXMasterInsertUpdateSingleItem] 
						@RecNo bigint = NULL,
...
BEGIN 
    ...
    BEGIN TRY
        ...
        IF (@OperationFlag = 0)
        BEGIN
                set @RecNo = (select count(*) from XMaster)
                if @RecNo  <> 0
                    set @RecNo = (select max(RecNo) from XMaster)
                set @RecNo = @RecNo + 1
        ...
        BEGIN
            DELETE FROM [XMaster] WHERE  [RecNo] = @RecNo
