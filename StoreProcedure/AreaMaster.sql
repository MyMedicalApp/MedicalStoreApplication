-- Store Procedures for AreaMaster Table
 
CREATE PROC [dbo].[sprocAreaMasterInsertUpdateSingleItem] 
...
BEGIN 
    ...
    BEGIN TRY
        ...
        IF (@OperationFlag = 0)
        BEGIN
                set @AreaCode = (select count(*) from AreaMaster)
                if @AreaCode  <> 0
                    set @AreaCode = (select max(AreaCode) from AreaMaster)
                set @AreaCode = @AreaCode + 1
                ...
                SET @Message=convert(varchar(20),@AreaCode)+' Record Inserted SucessFully'
        END
        ...
        BEGIN
                ...
                SET @Message=convert(varchar(20),@AreaCode)+' Record Update SucessFully'
        END
        ELSE IF (@OperationFlag = 2)
        BEGIN
            DELETE FROM [AreaMaster] WHERE  [AreaCode] = @AreaCode
            SET @Message=convert(varchar(20),@AreaCode)+' Record Deleted SucessFully'
        END
        COMMIT TRANSACTION
    END TRY
