-- Store Procedures for PurDetail Table
 
CREATE PROC [dbo].[sprocPurDetailInsertUpdateSingleItem] 
						@RecNo bigint = NULL,
...
BEGIN 
    ...
    BEGIN TRY
        ...
        IF (@OperationFlag = 0)
        BEGIN
                set @RecNo = (select count(*) from PurDetail)
                if @RecNo  <> 0
                    set @RecNo = (select max(RecNo) from PurDetail)
                set @RecNo = @RecNo + 1
 
                insert into [PurDetail]
                (
                    [BillTag],
                    [BillType],
                    [BillCredit],
        ...
        BEGIN
            DELETE FROM [PurDetail] WHERE  [RecNo] = @RecNo
