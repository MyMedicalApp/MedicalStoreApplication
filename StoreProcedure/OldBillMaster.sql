-- Store Procedures for OldBillMaster Table
 
CREATE PROC [dbo].[sprocOldBillMasterInsertUpdateSingleItem] 
						@RecNo bigint = NULL,
...
BEGIN 
    ...
    BEGIN TRY
        ...
        IF (@OperationFlag = 0)
        BEGIN
                set @RecNo = (select count(*) from OldBillMaster)
                if @RecNo  <> 0
                    set @RecNo = (select max(RecNo) from OldBillMaster)
                set @RecNo = @RecNo + 1
 
                insert into [OldBillMaster]
                (
                    [BillTag],
                    [BillType],
                    [BillCredit],
        ...
        BEGIN
            DELETE FROM [OldBillMaster] WHERE  [RecNo] = @RecNo
